...
const AdminDashboard = () => {
  ...
  const handleRoleChange = (username) => {
    // Toggle the role between 'admin' and 'member' when the checkbox is clicked
    setEditedUsers(editedUsers.map(user => 
      user.username === username ? { ...user, role: user.role === 'admin' ? 'member' : 'admin' } : user
    ));
  };
  ...
  return (
    <div>
      ...
      <Box sx={{ maxWidth: '600px', margin: '20px 0' }}>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={() => handleRoleChange(user.username)}
                  color="primary"
                />
              }
              label="Admin"
